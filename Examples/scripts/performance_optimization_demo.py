...
from ai_models.performance_optimizer import (
    CacheManager,
...
def demo_optimization():
    ...
    print("\nTest 2: Optimisation de maillage")
    print(
        f"Maillage original: {len(mesh1.vertices)} vertices, {len(mesh1.faces)} faces"
    )
    ...
    print(
        f"Résultat: {len(optimized_low.vertices)} vertices, {len(optimized_low.faces)} faces"
    )
    ...
    print(
        f"Résultat: {len(optimized_high.vertices)} vertices, {len(optimized_high.faces)} faces"
    )
