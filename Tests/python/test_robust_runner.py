...
class TestRobustSimulationRunner(unittest.TestCase):
    def setUp(self):
        ...
        self.runner = RobustSimulationRunner(
            max_workers=2, memory_threshold=90.0, timeout=60
        )
        self.valid_params = {
            "material": "PLA",
            "temperature": 200.0,
    ...
    def test_run_simulation_success(self, mock_process):
        ...
        with patch.object(
            self.runner, "_run_simulation_steps", side_effect=mock_run_steps
        ):
            results = self.runner.run_simulation(self.valid_params, callback)
    ...
    def test_handle_simulation_error(self, mock_log):
        ...
        error = SimulationError("Test error", "TEST_ERROR", {"step": "test"})
    ...
    def test_get_thermal_conductivity(self):
        ...
        self.assertEqual(self.runner._get_thermal_conductivity("PLA"), 0.13)
        self.assertEqual(self.runner._get_thermal_conductivity("ABS"), 0.17)
        self.assertEqual(
            self.runner._get_thermal_conductivity("UNKNOWN"), 0.15  # valeur par défaut
        )
