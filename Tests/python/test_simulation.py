...
from Python.simulation.fem_analysis import MaterialProperties, FEMAnalysis, analyze_model
from Python.simulation.thermal_sim import ThermalProperties, ThermalSimulation, simulate_thermal
...
class TestFEMAnalysis(unittest.TestCase):
    ...
    def test_invalid_material(self):
        ...
        result = analyze_model(
            str(self.test_mesh),
            material_name="INVALID_MATERIAL"
        )
    ...
    def test_extreme_forces(self):
        ...
        self.assertIn("recommendations", result)
        self.assertTrue(any("renforcer" in r.lower() for r in result["recommendations"]))
...
class TestThermalSimulation(unittest.TestCase):
    ...
    def test_extreme_temperature(self):
        ...
        result = simulate_thermal(
            str(self.test_mesh),
            material_name="PLA",
            initial_temp=material.melting_point + 50  # Bien au-dessus du point de fusion
        )
    ...
    def test_rapid_cooling(self):
        ...
        self.assertIn("recommendations", result)
        self.assertTrue(any("refroidissement" in r.lower() for r in result["recommendations"]))
        
    def test_time_step_limits(self):
        """Test les limites des pas de temps."""
        # Test avec très peu de pas de temps
        result1 = simulate_thermal(
            str(self.test_mesh),
            time_steps=2  # Trop peu de pas
        )
