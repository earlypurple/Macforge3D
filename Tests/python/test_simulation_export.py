...
class TestSimulationExporter(unittest.TestCase):
    ...
    def test_vtk_export(self):
        ...
        with tempfile.NamedTemporaryFile(suffix=".vtp") as tmp:
            success = SimulationExporter.export_to_vtk(
                self.mesh_data, self.results, tmp.name
            )
    ...
    def test_json_export(self):
        ...
        with tempfile.NamedTemporaryFile(suffix=".json") as tmp:
            # Test sans maillage
            success1 = SimulationExporter.export_to_json(self.results, tmp.name)
            ...
            success2 = SimulationExporter.export_to_json(
                self.results, tmp.name, include_mesh=True, mesh_data=self.mesh_data
            )
    ...
    def test_report_export(self):
        ...
        with tempfile.NamedTemporaryFile(suffix=".html") as tmp:
            success = SimulationExporter.export_report(
                self.results, tmp.name, title="Test Report"
            )
    ...
    def test_error_handling(self):
        ...
        success = SimulationExporter.export_to_json(
            self.results, "/nonexistent/path/file.json"
        )
        ...
        with tempfile.NamedTemporaryFile(suffix=".json") as tmp:
            success = SimulationExporter.export_to_json(
                {"invalid": float("nan")}, tmp.name  # JSON ne supporte pas NaN
            )
            self.assertFalse(success)
    ...
    def test_large_data(self):
        ...
        with tempfile.NamedTemporaryFile(suffix=".vtp") as tmp:
            success = SimulationExporter.export_to_vtk(
                large_mesh, self.results, tmp.name
            )
            self.assertTrue(success)
            self.assertTrue(Path(tmp.name).stat().st_size > 0)
