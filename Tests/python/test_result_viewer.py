...
class TestResultViewer(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.test_results = {
            ...
            "optimization_suggestions": ["Optimisation test 1", "Optimisation test 2"],
            "temperature_curve": {
                "times": [0, 1, 2, 3],
    ...
    def test_update_results(self):
        ...
        self.assertEqual(
            self.viewer.material_label.text(), self.test_results["material"]
        )
        self.assertEqual(self.viewer.status_label.text(), "Anomalies Détectées")
    ...
    def test_clear_functions(self):
        ...
        opt_count = sum(
            ...
            for i in range(self.viewer.optimizations_layout.count())
            if isinstance(
                self.viewer.optimizations_layout.itemAt(i).widget(), RecommendationItem
            )
        )
