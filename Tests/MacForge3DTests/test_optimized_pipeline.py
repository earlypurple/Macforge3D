...
class TestOptimizedPipeline(unittest.TestCase):
    ...
    def test_tensor_cache_and_detach(self):
        ...
        # Premier passage
        result = asyncio.run(
            self.pipeline.process_model(
                model,
                torch.utils.data.DataLoader(
                    torch.utils.data.TensorDataset(inputs), batch_size=2
                ),
                "test_model",
            )
        )
    ...
    def test_memory_optimization(self):
        ...
        self.assertLess(
            final_memory - initial_memory, 1e6
        )  # Moins de 1MB de différence
    ...
    def test_error_handling(self):
        ...
        with self.assertRaises(ValueError):
            ...
            asyncio.run(
                self.pipeline.generate("test", {}, ["missing_model"])  # Pas de modèles
            )
