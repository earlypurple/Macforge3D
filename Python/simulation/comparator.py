...
class SimulationComparator:
    ...
    def compare_multiple_results(
        self, results_list: List[Dict[str, Any]], metrics: Optional[List[str]] = None
    ) -> Dict[str, Any]:
        """
        ...
        try:
            if len(results_list) < 2:
                raise ValueError(
                    "Au moins deux ensembles de résultats sont nécessaires"
                )
            ...
            for i in range(len(results_list)):
                for j in range(i + 1, len(results_list)):
                    pair_comparison = self.compare_results(
                        results_list[i], results_list[j], metrics
                    )
                    comparison["pairwise_comparisons"].append(
                        {"pair": (i, j), "results": pair_comparison}
                    )
            ...
            comparison["clustering"] = self._cluster_results(results_list, metrics)

            # Analyse des tendances
            comparison["trends"] = self._analyze_trends(results_list, metrics)

            return comparison

        except Exception as e:
            logger.error(f"Erreur lors de la comparaison multiple: {str(e)}")
            raise

    def _compare_metric(
        self, values1: np.ndarray, values2: np.ndarray
    ) -> Dict[str, Any]:
        ...
        comparison["t_test"] = {"statistic": float(t_stat), "p_value": float(p_value)}
    ...
    def _find_common_metrics_multiple(
        ...
        all_metrics = [
            set(self._find_numeric_metrics(results)) for results in results_list
        ]
        return list(set.intersection(*all_metrics))

    def _find_numeric_metrics(
        self, results: Dict[str, Any], parent_key: str = ""
    ) -> List[str]:
        ...
        for key, value in results.items():
            ...
            if isinstance(value, (int, float, np.ndarray)):
                ...
                metrics.extend(self._find_numeric_metrics(value, current_key))
        return metrics

    def _extract_metric(self, results: Dict[str, Any], metric: str) -> np.ndarray:
    ...
    def _perform_statistical_tests(
        self, results1: Dict[str, Any], results2: Dict[str, Any], metrics: List[str]
    ) -> Dict[str, Any]:
    ...
    def _find_structural_differences(
        ...
        differences = {
            "missing_keys": {"results1": [], "results2": []},
            "type_differences": [],
            "shape_differences": [],
        }

        self._compare_structures(results1, results2, differences, path="")
    ...
    def _compare_structures(
        ...
        for key in keys1 & keys2:
            ...
            if type(value1) != type(value2):
                ...
                self._compare_structures(value1, value2, differences, current_path)
    ...
    def _compute_global_statistics(
        self, results_list: List[Dict[str, Any]], metrics: List[str]
    ) -> Dict[str, Any]:
        ...
        for metric in metrics:
            values = np.array(
                [self._extract_metric(results, metric) for results in results_list]
            )
