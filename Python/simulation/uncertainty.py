...
class UncertaintyAnalyzer:
        ...
        try:
            ...
            for i, params in enumerate(self.samples):
                try:
                    ...
                    if callback:
                        callback(
                            {"sample_index": i, "parameters": params, "results": result}
                        )

                except Exception as e:
                    logger.error(f"Erreur lors de la simulation {i}: {str(e)}")
    ...
    def _generate_samples(self, n_samples: int) -> List[Dict[str, Any]]:
        ...
        means = np.array([self.parameter_uncertainties[p][0] for p in param_names])
        stds = np.array([self.parameter_uncertainties[p][1] for p in param_names])

        # Générer les échantillons
        samples = np.random.normal(means, stds, size=(n_samples, len(param_names)))

        return [dict(zip(param_names, sample)) for sample in samples]
    ...
    def _analyze_results(self, metrics: List[str]) -> Dict[str, Any]:
        ...
        for metric, values in metric_values.items():
            ...
            ci = stats.norm.interval(0.95, loc=np.mean(values), scale=stats.sem(values))
        ...
        for metric in metrics:
            ...
            for param in self.parameter_uncertainties:
                param_values = np.array([sample[param] for sample in self.samples])

                # Corrélation de Spearman
                correlation, p_value = stats.spearmanr(param_values, values)
    ...
    def get_confidence_intervals(
        self, metric: str, confidence_level: float = 0.95
    ) -> Dict[str, float]:
        ...
        ci_param = stats.norm.interval(
            confidence_level, loc=np.mean(values), scale=stats.sem(values)
        )
        ...
        return {
            "parametric": {"lower": float(ci_param[0]), "upper": float(ci_param[1])},
            "bootstrap": {
