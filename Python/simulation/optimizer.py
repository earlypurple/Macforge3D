...
class SimulationOptimizer:
    ...
    def _bayesian_optimization(
        self, n_trials: int, callback: Optional[Callable[[Dict[str, Any]], None]]
    ) -> Dict[str, Any]:
        ...
        X_init = np.random.uniform(
            bounds[:, 0], bounds[:, 1], size=(n_initial, len(param_names))
        )
        y_init = np.array(
            [self._evaluate(dict(zip(param_names, x)), callback) for x in X_init]
        )
    ...
    def _nelder_mead(
        ...
        param_names = list(self.parameters_space.keys())
        x0 = np.array([np.mean(self.parameters_space[p]) for p in param_names])
    ...
    def _propose_next_point(
        self, X: np.ndarray, y: np.ndarray, bounds: np.ndarray, param_names: List[str]
    ) -> np.ndarray:
        ...
        X_random = np.random.uniform(
            bounds[:, 0], bounds[:, 1], size=(n_random, len(param_names))
        )
        ...
        for x0 in best_points:
            res = minimize(
                lambda x: -acquisition(x), x0, bounds=bounds, method="L-BFGS-B"
            )
    ...
    def get_optimization_summary(self) -> Dict[str, Any]:
        ...
        objective_values = [trial["objective_value"] for trial in self.history]

        return {
            "best_value": (
                max(objective_values) if self.maximize else min(objective_values)
            ),
