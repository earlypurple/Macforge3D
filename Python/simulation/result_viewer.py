...
class ResultGraph(pg.PlotWidget):
    ...
    def plot_temperature_curve(self, data: Dict[str, Any]):
        ...
        self.plot(
            times, max_temps, pen=pg.mkPen("r", width=2), name="Température maximale"
        )
        self.plot(
            times, min_temps, pen=pg.mkPen("b", width=2), name="Température minimale"
        )
...
class RiskIndicator(QWidget):
    ...
    def paintEvent(self, event):
        ...
        painter.drawText(
            self.rect(), Qt.AlignmentFlag.AlignCenter, self.risk_level.upper()
        )
...
class RecommendationItem(QFrame):
    ...
    def __init__(self, text: str, is_critical: bool = False, parent=None):
        ...
        icon = QLabel("●")
        icon.setStyleSheet(
            f"color: {'red' if is_critical else 'orange'}; " "font-size: 20px;"
        )
        ...
        if is_critical:
            ...
            self.setStyleSheet("background-color: #fff3e0; border: 1px solid #ffe0b2;")
...
class ResultViewer(QWidget):
    ...
    def update_results(self, results: Dict[str, Any]):
        ...
        try:
            ...
            self.status_label.setText(
                "Anomalies Détectées" if results.get("anomalies_detected") else "Normal"
            )
    ...
    def add_recommendation(self, text: str, is_critical: bool = False):
        ...
        self.recommendations_layout.insertWidget(
            self.recommendations_layout.count() - 1, item
        )
    ...
    def add_optimization(self, text: str):
        ...
        self.optimizations_layout.insertWidget(
            self.optimizations_layout.count() - 1, item
        )
