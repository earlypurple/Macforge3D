...
class MeshEnhancer:
    ...
    def _compute_vertex_normals(
        self, vertices: torch.Tensor, faces: torch.Tensor, face_normals: torch.Tensor
    ) -> torch.Tensor:
    ...
    def enhance_mesh(
        self, mesh: trimesh.Trimesh, return_original_scale: bool = True
    ) -> trimesh.Trimesh:
        """
        ...
        vertex_normals = self._compute_vertex_normals(vertices, faces, face_normals)

        # Préparer l'input pour l'encodeur
        x = torch.cat(
            [vertices, vertex_normals, torch.mean(edge_lengths[faces], dim=1)], dim=1
        )
        x = x.transpose(1, 2)
        ...
        with torch.no_grad():
            ...
            if self.config.smoothness_weight > 0:
                vertices = self._smooth_vertices(
                    vertices, faces, weight=self.config.smoothness_weight
                )
