...
import asyncio
from ai_models.optimized_pipeline import OptimizedPipeline, PipelineConfig, CacheConfig
...
class TestOptimizedPipeline(unittest.TestCase):
    ...
    def test_model_optimization(self):
        ...
        self.assertTrue(isinstance(optimized, torch.jit.RecursiveScriptModule))
    ...
    def test_cache_usage(self):
        ...
        np.testing.assert_array_almost_equal(outputs1.detach().numpy(), cached)
    ...
    async def test_process_model(self):
        ...
        results = await self.pipeline.process_model(model, data_loader, "test")
    ...
    async def test_generate(self):
        ...
        # Générer
        results = await self.pipeline.generate(test_input, models, steps)
        ...
        self.assertEqual(
            results["step1"].shape, (4, 7)
        )  # Batch size x output_size step1
        self.assertEqual(
            results["step2"].shape, (4, 5)
        )  # Batch size x output_size step2
...
TestOptimizedPipeline.test_generate = async_test(TestOptimizedPipeline.test_generate)
