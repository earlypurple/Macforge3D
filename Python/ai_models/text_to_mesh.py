...
def create_text_mesh(
    ...
    for char in text:
        ...
        try:
            ...
            vertices_2d = np.array([[x, y] for x, y in exterior[:, :2]], dtype=np.float64)
            # Process holes
            holes_2d = [np.array([[x, y] for x, y in hole[:, :2]], dtype=np.float64) for hole in interiors]
            
            print(f"  Creating mesh with polygon having {len(vertices_2d)} exterior points and {len(holes_2d)} holes...")
                ...
                try:
                    ...
                    if holes_2d:
                        ...
                        if len(vertices_2d) <= 4 or final_polygon.convex_hull.equals(final_polygon):
                            ...
                            triangles = [[0, i, i+1] for i in range(1, len(vertices_2d)-1)]
                        else:
                    ...
                    faces_side = []
                    for i in range(len(exterior) - 1):  # -1 car le dernier point est une copie du premier
                        v0 = i
