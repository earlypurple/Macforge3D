...
class OptimizedPipeline:
    ...
    def _setup_ray(self):
        ...
        if not ray.is_initialized() and self.config.use_parallel:
            ray.init(
                ...
                include_dashboard=self.config.profile
            )
        ...
        for i, batch in enumerate(data_loader):
            ...
            if isinstance(batch, (list, tuple)):
                ...
                futures.append(self._executor.submit(
                    lambda x: [item.to(self.config.device) if isinstance(item, torch.Tensor) else item for item in x],
                    batch
                ))
            else:
                # Sinon, convertir directement le batch
                futures.append(self._executor.submit(
                    lambda x: x.to(self.config.device) if isinstance(x, torch.Tensor) else x,
                    batch
                ))
    ...
    def _get_cached_result(
        ...
        key = f"{model_name}_{hash(input_tensor.cpu().numpy().tobytes())}"
    ...
    def _cache_result(
        ...
        key = f"{model_name}_{hash(inputs.detach().cpu().numpy().tobytes())}"
    ...
    async def process_model(
        ...
        for batch in data_loader:
            ...
            with torch.no_grad():
                input_tensor = input_tensor.detach().to(device=self.config.device, dtype=dtype)
                
            # Vérifier le cache
            cached = self._get_cached_result(model_name, input_tensor)
            if cached is not None:
                results.append(torch.from_numpy(cached).to(device=self.config.device, dtype=dtype))
                continue
                
            # Traiter en parallèle
            if self.config.use_parallel:
                result = await self._process_batch.remote(model, input_tensor)
            else:
                with torch.no_grad():
    ...
    async def generate(
        ...
        for step in steps:
            ...
            step_results = await self.process_model(
                models[step],
                data_loader,
                step
            )
    ...
    def optimize_memory(self):
        ...
        if self.config.profile:
            if torch.cuda.is_available():
                ...
                logger.info(f"CUDA memory cached: {torch.cuda.memory_reserved() / 1e6:.2f}MB")
    ...
    def profile_performance(self) -> Dict[str, float]:
        ...
        stats = {
            ...
            "ray_stats": ray.get_runtime_context().get_metrics()
            if self.config.use_parallel else {}
        }
