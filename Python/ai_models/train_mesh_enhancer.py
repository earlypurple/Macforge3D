...
class MeshDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, data_dir: str, max_points: int = 10000, augment: bool = True):
        """
        Args:
    ...
    def __getitem__(self, idx: int) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        if len(low_mesh.vertices) > self.max_points:
            indices = np.random.choice(
                len(low_mesh.vertices), self.max_points, replace=False
            )
            low_mesh.vertices = low_mesh.vertices[indices]
            if len(low_mesh.faces) > 0:
                valid_faces = np.all(np.isin(low_mesh.faces, indices), axis=1)
...
def train(
    ...
    for epoch in range(num_epochs):
        ...
        for low_vertices, high_vertices in tqdm(
            ...
            output = model.decoder(latent, low_vertices.shape[0])

            # Calcul de la perte
            reconstruction_loss = criterion(output[0].transpose(0, 1), high_vertices)
            distance_loss = chamfer_loss(
                output[0].transpose(0, 1).unsqueeze(0), high_vertices.unsqueeze(0)
            )
        ...
        with torch.no_grad():
            for low_vertices, high_vertices in val_loader:
                ...
                reconstruction_loss = criterion(
                    output[0].transpose(0, 1), high_vertices
                )
                distance_loss = chamfer_loss(
                    output[0].transpose(0, 1).unsqueeze(0), high_vertices.unsqueeze(0)
                )
...
if __name__ == "__main__":
    ...
    train_dataset = MeshDataset("data/train", max_points=10000, augment=True)
    val_dataset = MeshDataset("data/val", max_points=10000, augment=False)
