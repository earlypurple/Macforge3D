...
# Ces imports n'ont pas de stubs disponibles
import zstandard
import lz4.frame
import blosc2
...
class SmartCache:
    ...
    def _compress_data(self, data: bytes) -> bytes:
        ...
        if self.config.compression_algorithm == "zstd":
            ...
            return blosc2.compress(
                data, clevel=self.config.compression_level, typesize=8
            )
        else:
    ...
    def _store_in_mmap(self, key: str, data: Any) -> bool:
        ...
        try:
            ...
            data_bytes = (
                data.tobytes() if isinstance(data, np.ndarray) else str(data).encode()
            )
    ...
    def cleanup(self):
        ...
        with self._lock:
            ...
            if self._memory_usage > self.config.max_memory_size:
                items = sorted(
                    self._memory_cache.items(), key=lambda x: self._get_data_size(x[1])
                )
                while self._memory_usage > self.config.max_memory_size * 0.8 and items:
                    key, data = items.pop()
            ...
            if self._disk_usage > self.config.max_disk_size:
                cache_files = sorted(
                    self._disk_cache_path.glob("*"), key=lambda x: x.stat().st_mtime
                )
                while (
                    self._disk_usage > self.config.max_disk_size * 0.8 and cache_files
                ):
    ...
    def put(
        self, data: Any, key: Optional[str] = None, force_disk: bool = False
    ) -> str:
        """
