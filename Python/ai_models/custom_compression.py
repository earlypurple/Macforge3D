...
try:
    ...
    print("⚠️  h5py not available. HDF5 compression will be disabled.")
...
class MeshCompressor:
    ...
    def __init__(self, format_config: Optional[CompressionFormat] = None):
    ...
    def train_autoencoder(
        ...
        # Créer le modèle
        self.autoencoder: MeshAutoencoder = MeshAutoencoder()
    ...
    def compress_mesh(
        self, mesh: Any, output_path: Optional[Path] = None
    ) -> Union[bytes, Path]:
        """
        ...
        if self.format.parameters["use_octree"]:
            ...
            self.kdtree: cKDTree = cKDTree(vertices)
            self.vertex_clusters: defaultdict = defaultdict(list)

            threshold = self.format.parameters["cluster_threshold"]
            for i, v in enumerate(vertices):
                if self.kdtree is not None:
                    neighbors = self.kdtree.query_ball_point(v, threshold * v_range.mean())
                    cluster_id = min(neighbors)
                    if self.vertex_clusters is not None:
        ...
        if output_path:
            ...
            compressed_data = {
                ...
                "attributes": {
                    name: data.tobytes() for name, data in preserved_data.items()
                },
            }
    ...
    def decompress_mesh(self, data: Union[bytes, Path]) -> Any:
        """
        ...
        if isinstance(data, (str, Path)):
            ...
            with h5py.File(data, "r") as f:
                ...
                if (
                    ...
                    or format_version != self.format.version
                ):
                    raise ValueError(
                        f"Format incompatible: {format_name} {format_version}"
                    )
            ...
            if (
                format_info["name"] != self.format.name
                or format_info["version"] != self.format.version
            ):
                raise ValueError(
            ...
            vertices_quantized = np.frombuffer(
                geometry["vertices_quantized"].encode(), dtype=np.float32
            ).reshape(-1, 3)

            v_min = np.frombuffer(geometry["v_min"].encode(), dtype=np.float32)
            v_max = np.frombuffer(geometry["v_max"].encode(), dtype=np.float32)

            faces = np.frombuffer(geometry["faces"].encode(), dtype=np.int32).reshape(
                -1, 3
            )
            ...
            for name, data in compressed_data.get("attributes", {}).items():
                attributes[name] = np.frombuffer(data.encode(), dtype=np.float32)

            # Reconstruire avec l'autoencoder si disponible
            if "latent" in compressed_data and self.autoencoder is not None:
                latent = torch.FloatTensor(
                    np.frombuffer(compressed_data["latent"].encode(), dtype=np.float32)
                )
                with torch.no_grad():
                    vertices = self.autoencoder.decode(latent).numpy()
