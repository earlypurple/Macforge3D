...
try:
    ...
    # Fallback silencieux - pas d'avertissement car ce n'est pas critique
...
class CacheProfiler:
    ...
    def __init__(self):
        ...
        self.operation_counters: Counter = Counter()
    ...
    def get_stats(self, operation: Optional[str] = None) -> Dict[str, Any]:
        ...
        compression_ratios = [
            p.compression_ratio for p in profiles if p.compression_ratio is not None
        ]
...
class CacheCompressor:
    ...
    def __init__(self, config: Optional[CompressionConfig] = None):
        ...
        # Initialiser les compresseurs si disponibles
        if ZSTD_AVAILABLE:
            self.zstd_compressor = zstd.ZstdCompressor(
                level=self.config.level, threads=self.config.compression_threads
            )
    ...
    def compress(self, data: bytes) -> Tuple[bytes, float]:
        ...
        if self.config.algorithm == "zstd" and ZSTD_AVAILABLE:
            compressed = self.zstd_compressor.compress(data)
        elif self.config.algorithm == "lz4" and LZ4_AVAILABLE:
            compressed = lz4.frame.compress(data, compression_level=self.config.level)
        elif self.config.algorithm == "blosc" and BLOSC2_AVAILABLE:
            compressed = blosc2.compress(data, clevel=self.config.level, typesize=8)
        else:
    ...
    def compress_numpy_array(self, array: np.ndarray) -> Tuple[bytes, Dict[str, Any]]:
        """
    ...
    def decompress_numpy_array(
        ...
        return np.frombuffer(decompressed, dtype=np.dtype(metadata["dtype"])).reshape(
            metadata["shape"]
        )

    def compress_mesh(self, mesh: Any) -> Tuple[bytes, Dict[str, Any]]:
        """
    ...
    def decompress_mesh(self, data: bytes, metadata: Dict[str, Any]) -> Any:
        """
        ...
        vertices = np.frombuffer(
            self.decompress(vertices_data), dtype=np.float32
        ).reshape(metadata["vertices_shape"])

        faces = np.frombuffer(self.decompress(faces_data), dtype=np.int32).reshape(
            metadata["faces_shape"]
        )
