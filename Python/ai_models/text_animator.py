...
class TextAnimator:
    ...
    def add_animation(self, name: str, animation: AnimationClip):
    ...
    def create_rotation_animation(
        ...
        for t, angle in zip(times, angles):
            ...
            # Récupérer la matrice et garantir qu'elle est un ndarray standard
            rot_matrix: np.ndarray = np.array(rot.as_matrix())
            # Aplatir en 1D
            flat_matrix = rot_matrix.flatten()
            keyframes.append(AnimationKeyframe(time=t, value=flat_matrix))
    ...
    def create_wave_animation(
        ...
        for t in times:
            offset = (
                direction * amplitude * np.sin(2 * np.pi * frequency * t / duration)
            )
            keyframes.append(AnimationKeyframe(time=t, value=offset))

        return AnimationClip(
            type=AnimationType.WAVE, keyframes=keyframes, duration=duration, loop=loop
        )
    ...
    def create_bounce_animation(
        ...
        for t in times:
            ...
            keyframes.append(AnimationKeyframe(time=t, value=np.array([0, y, 0])))

        return AnimationClip(
            type=AnimationType.BOUNCE, keyframes=keyframes, duration=duration, loop=loop
        )

    def _interpolate_keyframes(
        self, keyframes: List[AnimationKeyframe], current_time: float, loop: bool
    ) -> np.ndarray:
    ...
    def apply_animations(
        self, mesh: trimesh.Trimesh, delta_time: float
    ) -> trimesh.Trimesh:
        """
        ...
        for anim in self.animations.values():
            ...
            if anim.type == AnimationType.ROTATION:
                ...
                rot_matrix = self._interpolate_keyframes(
                    anim.keyframes, self.current_time, anim.loop
                ).reshape(3, 3)
    ...
    def save_animations(self, filepath: str):
        ...
        data = {
            name: {
                ...
                "keyframes": [
                    {"time": kf.time, "value": kf.value.tolist(), "easing": kf.easing}
    ...
    def load_animations(self, filepath: str):
        ...
        for name, anim_data in data.items():
            keyframes = [
                AnimationKeyframe(
                    time=kf["time"], value=np.array(kf["value"]), easing=kf["easing"]
                )
...
class GPUMeshProcessor:
    ...
    def process_mesh(
        self, mesh: trimesh.Trimesh, compute_normals: bool = True
    ) -> torch.Tensor:
        """
        ...
        if compute_normals:
            ...
            # Ajouter les normales aux attributs - utiliser une copie pour éviter l'erreur d'affectation à une méthode
            normals_numpy = vertex_normals.detach().cpu().numpy()
            # Affecter de façon sûre
            if hasattr(mesh, '_vertex_normals'):
                mesh._vertex_normals = normals_numpy
            else:
                # Fallback pour autres versions de trimesh
                setattr(mesh, 'vertex_normals', normals_numpy)

        return vertices

    def apply_transform(
        self, vertices: torch.Tensor, transform: np.ndarray
    ) -> torch.Tensor:
        """Applique une transformation sur GPU."""
        # Convertir la transformation en tenseur torch
        transform_tensor = torch.tensor(transform, dtype=torch.float32, device=self.device)

        # Appliquer la rotation et l'échelle
        rotated = torch.matmul(vertices, transform_tensor[:3, :3].T)

        # Appliquer la translation
        translated = rotated + transform_tensor[:3, 3]

        return translated

    def compute_morphing(
        self, source_vertices: torch.Tensor, target_vertices: torch.Tensor, t: float
    ) -> torch.Tensor:
