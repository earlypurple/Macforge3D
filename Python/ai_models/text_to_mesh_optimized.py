    ...
    def _moveTo(self, pt):
        if self.current_contour:
            self.contours.append(
                self._points_buffer[:self._buffer_index].copy()
            )
        self._reset_buffer()
        self._add_point(pt)
    ...
    def _closePath(self):
        if self._buffer_index > 0:
            if not np.array_equal(
                self._points_buffer[0],
                self._points_buffer[self._buffer_index - 1]
            ):
...
def process_glyph(
    ...
    try:
        ...
        if len(vertices_2d) <= 4 or final_polygon.convex_hull.equals(final_polygon):
            ...
            triangles = np.array([[0, i, i+1] for i in range(1, len(vertices_2d)-1)])
        else:
        ...
        faces_side = np.column_stack((
            np.column_stack((v0, v1, v2)),
            np.column_stack((v2, v3, v0))
        )).reshape(-1, 3)
...
@lru_cache_decorator.lru_cache(maxsize=128)
...
def create_text_mesh(
    ...
    try:
        ...
        for char in text:
            ...
            glyph_data.append((
                char, glyph, scale, depth, glyph_name, use_cache
            ))
