...
class TextEffects:
    ...
    def apply_style(self, mesh: trimesh.Trimesh, style: TextStyle) -> trimesh.Trimesh:
        ...
        if style.bevel_amount > 0:
            result = self._apply_bevel(result, style.bevel_amount, style.bevel_segments)

        if style.noise_scale > 0:
            result = self._apply_noise(result, style.noise_scale)

        if style.distortion > 0:
            result = self._apply_distortion(result, style.distortion)

        # Ajouter les attributs de matériau
        result.visual = trimesh.visual.TextureVisuals()

        if style.texture_path:
            # Utiliser PIL directement pour charger l'image car trimesh.load_image n'existe pas
            from PIL import Image
            result.visual.material.image = Image.open(style.texture_path)

        if style.normal_map_path:
            # Utiliser PIL directement pour charger l'image car trimesh.load_image n'existe pas
            from PIL import Image
            # Mettre l'image dans le material kwargs
            result.visual.material.kwargs['normal_image'] = Image.open(style.normal_map_path)
    ...
    def _apply_bevel(
        self, mesh: trimesh.Trimesh, amount: float, segments: int
    ) -> trimesh.Trimesh:
        ...
        for edge in edges:
            ...
            if len(face_idx) == 2:  # Arête intérieure
                angle = np.arccos(np.dot(normals[face_idx[0]], normals[face_idx[1]]))
    ...
    def _apply_noise(self, mesh: trimesh.Trimesh, scale: float) -> trimesh.Trimesh:
        """Applique un bruit procédural au maillage."""
        # Générer du bruit de Perlin 3D
        from noise import snoise3  # type: ignore
        ...
        for i in range(len(vertices)):
            pos = vertices[i]
            noise = snoise3(pos[0] * scale, pos[1] * scale, pos[2] * scale)
            vertices[i] += normals[i] * noise * scale

        return trimesh.Trimesh(vertices=vertices, faces=mesh.faces)

    def _apply_distortion(
        self, mesh: trimesh.Trimesh, amount: float
    ) -> trimesh.Trimesh:
        ...
        scale: float = np.max(np.abs(vertices - center))
...
PREDEFINED_STYLES = {
    "standard": TextStyle(
        name="standard", bevel_amount=0.0, roughness=0.5, color=(0.8, 0.8, 0.8)
    ),
    "moderne": TextStyle(
        name="moderne",
        bevel_amount=0.1,
    ...
    "or": TextStyle(
        name="or", bevel_amount=0.08, roughness=0.3, metallic=1.0, color=(1.0, 0.8, 0.0)
    ),
    "pierre": TextStyle(
        name="pierre", noise_scale=0.2, roughness=0.9, color=(0.6, 0.6, 0.6)
    ),
    "liquide": TextStyle(
        name="liquide",
        distortion=0.2,
