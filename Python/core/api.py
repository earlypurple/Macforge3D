...
async def get_current_metrics():
    ...
    try:
        ...
        return {"timestamp": datetime.now(), "metrics": system_metrics.__dict__}
...
async def get_metrics_history(hours: int = 1, interval: str = "1m"):
    ...
    try:
        ...
        grouped_metrics = {}
        for m in metrics:
            timestamp = m.timestamp.replace(second=0, microsecond=0)
...
async def get_simulations(hours: int = 24, status: Optional[str] = None):
    ...
    try:
        ...
        simulations = [s for s in simulations if s.start_time >= cutoff]
...
async def get_alerts(severity: Optional[str] = None, hours: int = 24):
    ...
    try:
        return [
            a.__dict__
            for a in performance_monitor.metrics_buffer.get_alerts(severity, hours)
        ]
    except Exception as e:
        raise HTTPException(
            status_code=500,
...
app.mount(
    "/dashboard", StaticFiles(directory="static/dashboard", html=True), name="dashboard"
)
