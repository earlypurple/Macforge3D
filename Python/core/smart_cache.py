...
class SmartCache:
    ...
    def put(self, parameters: Dict[str, Any], result: Any, computation_time: float):
    ...
    def _compute_key(self, parameters: Dict[str, Any]) -> str:
        ...
        return hashlib.sha256(json.dumps(sorted_params).encode()).hexdigest()
    ...
    def _cleanup_if_needed(self, new_size: int):
        ...
        while (
            len(self.metadata) >= self.max_entries
            or current_size + new_size > self.max_size_bytes
        ):
    ...
    def _update_predictor(self, parameters: Dict[str, Any], computation_time: float):
        ...
        try:
            ...
            joblib.dump(
                self.time_predictor, os.path.join(self.meta_dir, "predictor.joblib")
            )
            joblib.dump(self.time_scaler, os.path.join(self.meta_dir, "scaler.joblib"))
