...
class MLOptimizer:
    ...
    def _objective(self, trial):
        ...
        for goal in self.goals:
            ...
            if (
                goal.constraint_min is not None and
                predicted < goal.constraint_min
            ):
                return float('-inf')
                
            if (
                goal.constraint_max is not None and
                predicted > goal.constraint_max
            ):
                return float('-inf')
        ...
        for goal in self.goals:
            ...
            models = {
                ...
                'gp': GaussianProcessRegressor(
                    kernel=C(1.0) * RBF([1.0] * X.shape[1]),
                    random_state=42
                )
            }
            ...
            for name, model in models.items():
                try:
                    ...
                    logger.warning(
                        f"Erreur validation croisée {name}: {str(e)}"
                    )
    ...
    def _predict_metric(
        ...
        if isinstance(self.models[metric], GaussianProcessRegressor):
            pred, std = self.models[metric].predict(
                X_scaled,
                return_std=True
            )
            return float(pred[0])
